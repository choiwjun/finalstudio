...
## 다음 작업 핸드오프 — 키워드·주제 조사, 수집, 분석

### 목표

AI·IT, 경제, 건강 세 상위 카테고리의 키워드와 주제를 먼저 조사한 뒤, 실제로 근거를 확보할 수 있는 글 목록을 만든다. 키워드 조사 결과만으로 글을 자동 생성하거나 자동 발행하지 않는다.

### 조사 원칙

- 상위 키워드는 카테고리와 주제 지도를 정하는 기준으로 사용한다.
- 각 글은 대표 키워드 1개와 관련 키워드 2~5개를 갖는다.
- 키워드마다 검색 의도를 `방법`, `개념`, `비교`, `문제 해결`, `최신 이슈` 중 하나 이상으로 분류한다.
- 검색량이나 경쟁도를 데이터 없이 추정하지 않는다.
- 검색 결과 페이지를 무단으로 수집하지 않는다. 공식 API, 허용된 데이터 도구, 사용자가 내려받은 CSV·JSON을 우선한다.
- AI·IT는 버전과 기준일을 기록한다. 경제·건강은 공식 출처·기준일·최신성을 필수로 기록한다.
- 경제 글은 투자 추천·수익 보장을 하지 않는다. 건강 글은 진단·치료·약물 처방을 하지 않는다. 위험 신호가 있으면 사람 검토를 요구한다.

### 조사 대상

1. **AI·IT** (`ai-it`)
   - AI 활용, 개발·자동화, 업무도구, 블로그 운영
2. **경제** (`economy`)
   - 경제 개념, 정책, 공공 통계, 생활금융
3. **건강** (`health`)
   - 생활 습관, 수면·운동·기록, 공식 보건 자료 해설

여행·독서·사진·경험 형식은 상위 카테고리로 확장하지 않고, 실제 원자료가 있을 때만 보조 주제로 사용한다.

### 수집 입력

- 사용자가 제공한 seed keyword 목록
- Google Search Console 내보내기 데이터
- Google Keyword Planner 등 공식·허용된 키워드 데이터
- 네이버 DataLab 등 공식 데이터
- 공식 문서·공공기관·보건기관·통계 출처

키워드 데이터 제공처와 수집일을 각 결과에 함께 저장한다.
...
`status` 값은 `candidate`, `researching`, `ready-to-write`, `written`, `rejected`로 관리한다.

### 결과물

- `HANDOFF.md`: 조사 결정과 우선순위 요약
- 키워드 맵: 카테고리별 상위 키워드·세부 키워드·검색 의도·근거
- 주제 백로그: 실제 작성 가능한 글 제목·대표 키워드·관련 키워드·출처
- 글별 브리프: 독자 문제, 핵심 답변, 출처, 기준일, 검증 항목

키워드 맵과 주제 백로그의 저장 위치와 파일 형식은 조사 시작 전에 결정한다.

### 실행 순서

1. 세 카테고리별 seed keyword를 확정한다.
2. 허용된 출처에서 키워드와 관련 주제를 수집한다.
3. 중복·동의어·너무 넓은 키워드를 정리한다.
4. 검색 의도와 실제 작성 가능성을 분석한다.
5. 카테고리별 우선 주제 5~10개를 선정한다.
6. 실제 게시할 첫 글의 브리프를 만든다.
7. 글 작성 후 해당 글만 writing check·출처 확인·사람 검토를 수행한다.

전체 synthetic 평가 케이스 8건은 이 작업의 일부가 아니며, 사용자가 별도로 요청하지 않는 한 실행하지 않는다.
