...
if (!slug)
  fail(
    "--slug <영어슬러그> 가 필요합니다. 예: npm run image -- --slug excel-linked-picture",
  );

const postPath = join(POSTS_DIR, `${slug}.md`);
if (!existsSync(postPath))
  fail(`글 파일이 없습니다: ${postPath} — 글을 먼저 저장하세요.`);
...
const setImageFrontmatter = () => {
  ...
  try {
    execFileSync(
      process.execPath,
      [join(ROOT, "node_modules", "astro", "bin", "astro.mjs"), "sync"],
...
if (ENGINE === "manual") {
  ...
  console.log(
    "  1) ChatGPT(구독 플랜)에 아래 프롬프트를 붙여넣어 이미지를 생성하세요:\n",
  );
  console.log(
    `--- 이미지 프롬프트 (복사) ---\n${imagePrompt}\n--- 여기까지 ---\n`,
  );
...
if (!CODEX_COMMAND)
  fail(
    "codex CLI가 없습니다. `npm install -g @openai/codex` 후 `codex login`을 실행하세요.",
  );
...
try {
  ...
  fail(
    error instanceof Error
      ? error.message
      : "Codex 이미지 생성에 실패했습니다.",
  );
}
if (!existsSync(targetPath))
  fail(`Codex가 이미지를 저장하지 않았습니다: ${targetPath}`);
