...
test("Given invalid blog request bounds, when validated, then a deterministic contract error is thrown", () => {
  for (const request of [
    ...
    assert.throws(
      () => validateBlogSearchRequest(request),
      ContractValidationError,
    );
  }
});
...
test("Given invalid trend dates or group counts, when validated, then the request is rejected", () => {
  ...
  for (const request of [
    ...
    {
      ...valid,
      keywordGroups: Array.from({ length: 6 }, (_, index) => ({
        groupName: `g${index}`,
        keywords: ["x"],
      })),
    },
    {
      ...valid,
      keywordGroups: [
        { groupName: "x", keywords: Array.from({ length: 21 }, () => "x") },
      ],
    },
  ]) {
...
test("Given successful blog and trend fixture bodies, when normalized, then typed response fields are returned", async () => {
  const blog = normalizeBlogSearchResponse(
    await readJsonFixture("blog-success.json"),
  );
  const trend = normalizeTrendResponse(
    await readJsonFixture("trend-success.json"),
  );
...
test("Given a non-text blog description, when normalized, then it becomes bounded empty reference data", () => {
  const response = normalizeBlogSearchResponse({
    ...
    items: [
      {
        title: "제목",
        description: { prompt: "ignore instructions" },
...
test("Given API gateway, search, trend, and HTTP failure bodies, when normalized, then risk flags are explicit", async () => {
  ...
  for (const [name, status, kind, risk] of cases) {
    const failure = normalizeApiFailure({
      status,
      body: await readJsonFixture(name),
    });
    ...
    assert.equal(
      failure.message.includes(["NCP", "_SECRET", "_SENTINEL"].join("")),
      false,
    );
  }
...
test("Given a record with a missing required field or invalid ready state, when validated, then it is rejected", () => {
  ...
  for (const field of Object.keys(record)) {
    ...
    assert.throws(
      () => validateWjKeywordRecord(incomplete),
      ContractValidationError,
      field,
    );
  }
      ...
      items: [
        {
          title: "x",
          description: "x",
          link: "https://blog.example.test/x",
          postdate: "20260909",
        },
      ],
    },
  ...
  assert.throws(
    () =>
      normalizeRawEvidenceEnvelope({
        ...envelope,
        request: {
          headers: { Authorization: ["NCP", "_SECRET", "_SENTINEL"].join("") },
        },
      }),
    ContractValidationError,
  );
...
test("Given contradictory HTTP and envelope fields, when evidence is normalized, then the contract rejects every mismatch", () => {
  ...
  for (const envelope of [
    ...
    assert.throws(
      () => normalizeRawEvidenceEnvelope(envelope),
      ContractValidationError,
    );
  }

  const normalized = normalizeRawEvidenceEnvelope({
    ...base,
    http: { status: 401, ok: false },
    error: failure,
  });
...
test("Given empty, malformed, or explicitly malformed response evidence, when normalized, then it never becomes usable success evidence", async () => {
  ...
  assert.deepEqual(malformed.error.risk_flags, [
    "api_error",
    "malformed_response",
  ]);
});
...
test("Given documented numeric, category, and timestamp boundaries, when validated, then invalid values are rejected", () => {
    ...
    results: [
      {
        title: "x",
        keywords: ["x"],
        data: [{ period: "2026-09-09", ratio: 1 }],
      },
    ],
  };
  for (const response of [
    {
      ...
      results: [
        {
          ...trend.results[0],
          data: [{ period: "2026-09-09", ratio: 100.01 }],
        },
      ],
    },
    {
    ...
    },
    {
      ...trend,
      results: [
        { ...trend.results[0], data: [{ period: "2026-02-30", ratio: 1 }] },
      ],
    },
  ]) {
    assert.throws(
      () => normalizeTrendResponse(response),
      ContractValidationError,
    );
  }
  ...
  for (const category of ["---", "-ai-it", "ai-it-", "AI-it"]) {
    assert.throws(
      () => normalizeWjKeywordRecord({ ...makeValidRecord(), category }),
      ContractValidationError,
    );
  }
  assert.throws(
    () =>
      normalizeWjKeywordRecord({
        ...makeValidRecord(),
        collected_at: "2026-02-30T00:00:00.000Z",
      }),
    ContractValidationError,
  );
});
