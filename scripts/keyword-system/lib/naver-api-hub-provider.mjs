...
function readCredentials(env) {
  const clientId =
    typeof env[CLIENT_ID_ENV] === "string" ? env[CLIENT_ID_ENV].trim() : "";
  const clientSecret =
    typeof env[CLIENT_SECRET_ENV] === "string"
      ? env[CLIENT_SECRET_ENV].trim()
      : "";
...
function buildBlogUrl(request) {
  ...
  for (const key of BLOG_REQUEST_FIELDS)
...
async function run(searchKind, request, state) {
  ...
  const url =
    searchKind === "blog"
      ? buildBlogUrl(request)
      : new URL(TREND_ENDPOINT, NAVER_API_HUB_BASE_URL);
  const init =
    searchKind === "blog"
  ...
  try {
    ...
    if (error?.code === "NAVER_RESPONSE_TOO_LARGE")
      return responseTooLargeFailure(response.status, redactionValues);
    return networkFailure(error, state.timeoutMs, redactionValues);
  }
...
export function createNaverApiHubProvider(options = {}) {
  ...
  if (typeof fetchImpl !== "function")
    throw new TypeError("createNaverApiHubProvider: fetchImpl is required");
  if (!Number.isInteger(timeoutMs) || timeoutMs <= 0)
    throw new TypeError(
      "createNaverApiHubProvider: timeoutMs must be a positive integer",
    );
