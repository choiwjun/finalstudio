...
export const normalizeKeywordKey = (value) =>
  String(value ?? "")
    .normalize("NFC")
    .replace(/\s+/gu, " ")
    .trim()
...
const normalizedText = (value, path) =>
  string(value, path).normalize("NFC").replace(/\s+/gu, " ").trim();
const boundedNormalizedText = (value, path, maximum) => {
  const normalized = normalizedText(value, path);
  if (normalized.length > maximum)
    fail(path, `must be ${maximum} characters or fewer`);
  return normalized;
};
...
const isoDateTime = (value, path) => {
  ...
  if (
    ...
    fail(path, "must be an ISO UTC date-time");
...
const enumValue = (value, values, path) =>
  values.includes(value) ? value : fail(path, "must be an allowed value");
const optionalEnum = (value, values, path) =>
  value === undefined ? undefined : enumValue(value, values, path);
...
export function normalizeTrendRequest(input) {
  ...
  if (startDate > endDate)
    fail("request", "startDate must not be after endDate");
  const groups = Array.isArray(value.keywordGroups)
    ? value.keywordGroups
    : fail("request.keywordGroups", "must be an array");
  ...
  const keywordGroups = groups.map((group, index) => {
    ...
    const keywords = Array.isArray(item.keywords)
      ? item.keywords
      : fail(`request.keywordGroups[${index}].keywords`, "must be an array");
    integer(
      keywords.length,
      `request.keywordGroups[${index}].keywords.length`,
      1,
      20,
    );
    return {
      groupName: boundedNormalizedText(
        item.groupName,
        `request.keywordGroups[${index}].groupName`,
        MAX_QUERY_TEXT_LENGTH,
      ),
      keywords: keywords.map((keyword, keywordIndex) =>
        boundedNormalizedText(
          keyword,
          `request.keywordGroups[${index}].keywords[${keywordIndex}]`,
          MAX_QUERY_TEXT_LENGTH,
        ),
      ),
    };
  ...
  const result = {
    startDate,
    endDate,
    timeUnit: enumValue(value.timeUnit, TREND_TIME_UNITS, "request.timeUnit"),
    keywordGroups,
  };
  for (const field of ["device", "gender"]) {
    const normalized = optionalEnum(
      value[field],
      field === "device" ? ["pc", "mo"] : ["m", "f"],
      `request.${field}`,
    );
    if (normalized !== undefined) result[field] = normalized;
  }
  if (value.ages !== undefined) {
    if (!Array.isArray(value.ages) || value.ages.length > 11)
      fail("request.ages", "must be an array of up to 11 age values");
    result.ages = value.ages.map((age, index) =>
      enumValue(String(age), [...AGE_VALUES], `request.ages[${index}]`),
    );
  }
  return result;
}
...
function normalizeBlogItem(item, index) {
  ...
  return {
    title: stripBold(
      boundedNormalizedText(
        value.title,
        `response.items[${index}].title`,
        MAX_BLOG_TITLE_LENGTH,
      ),
    ),
    description: stripBold(
      optionalBoundedNormalizedText(
        value.description,
        `response.items[${index}].description`,
        MAX_BLOG_DESCRIPTION_LENGTH,
      ),
    ),
    link: boundedNormalizedText(
      value.link,
      `response.items[${index}].link`,
      MAX_BLOG_LINK_LENGTH,
    ),
...
export function normalizeBlogSearchResponse(input) {
  ...
  const items = Array.isArray(value.items)
    ? value.items
    : fail("response.items", "must be an array");
  if (items.length > MAX_BLOG_ITEMS)
    fail("response.items", `must contain ${MAX_BLOG_ITEMS} items or fewer`);
  ...
  for (const [index] of items.entries()) {
    ...
    if (!/^\d{8}$/u.test(item.postdate))
      fail(`response.items[${index}].postdate`, "must be YYYYMMDD");
      ...
      result[field] =
        field === "lastBuildDate"
          ...
          : integer(
              ...
              `response.${field}`,
              field === "start" ? 1 : 1,
              field === "start" ? 1000 : 100,
            );
    }
  }
  if (normalizedItems.length > value.total)
    fail("response.items", "must not contain more items than total");
  if (result.display !== undefined && normalizedItems.length > result.display)
    fail("response.items", "must not exceed response.display");
  return result;
}

export const validateBlogSearchResponse = normalizeBlogSearchResponse;
export const isValidBlogSearchResponse = (value) => {
  try {
    normalizeBlogSearchResponse(value);
    return true;
  } catch {
    return false;
  }
...
function normalizeTrendData(data, resultIndex) {
  ...
  if (data.length > MAX_TREND_DATA_POINTS)
    fail(
      ...
      `must contain ${MAX_TREND_DATA_POINTS} points or fewer`,
    );
  return data.map((item, index) => {
    const value = object(
      item,
      `response.results[${resultIndex}].data[${index}]`,
    );
    const period = date(
      value.period,
      `response.results[${resultIndex}].data[${index}].period`,
    );
    if (
      ...
      fail(
        `response.results[${resultIndex}].data[${index}].ratio`,
        "must be a number in range 0..100",
      );
...
export function normalizeTrendResponse(input) {
  ...
  if (startDate > endDate)
    fail("response", "startDate must not be after endDate");
  if (!Array.isArray(value.results))
    fail("response.results", "must be an array");
  if (value.results.length > MAX_TREND_RESULTS)
    fail(
      "response.results",
      `must contain ${MAX_TREND_RESULTS} results or fewer`,
    );
  const results = value.results.map((result, index) => {
    const item = object(result, `response.results[${index}]`);
    const keywords = Array.isArray(item.keywords)
      ? item.keywords
      : fail(`response.results[${index}].keywords`, "must be an array");
    if (keywords.length > MAX_TREND_KEYWORDS)
      fail(
        `response.results[${index}].keywords`,
        `must contain ${MAX_TREND_KEYWORDS} keywords or fewer`,
      );
    if (keywords.length === 0)
      fail(`response.results[${index}].keywords`, "must not be empty");
    return {
      title: boundedNormalizedText(
        item.title,
      ...
      keywords: keywords.map((keyword, keywordIndex) =>
        boundedNormalizedText(
          keyword,
          `response.results[${index}].keywords[${keywordIndex}]`,
          MAX_QUERY_TEXT_LENGTH,
        ),
      ),
      data: normalizeTrendData(item.data, index),
    };
  });
  return {
    startDate,
    endDate,
    timeUnit: enumValue(value.timeUnit, TREND_TIME_UNITS, "response.timeUnit"),
    results,
  };
}

export const validateTrendResponse = normalizeTrendResponse;
export const isValidTrendResponse = (value) => {
  try {
    normalizeTrendResponse(value);
    return true;
  } catch {
    return false;
  }
...
const normalizeRedactionValues = (values) =>
  [
    ...new Set(
...
const redactText = (value, redactionValues = []) => {
  ...
  for (const secret of normalizeRedactionValues(redactionValues))
    text = text.split(secret).join("[redacted]");
  return text
    .replace(
      /((?:authorization|proxy-authorization|x-ncp-[^\s:=]+|api[-_ ]?key|client[-_ ]?(?:id|secret)|access[-_]?token|refresh[-_]?token|password|credential|secret|token)\s*[:=]\s*)(?:(?:bearer|basic)\s+[^\s,;&]+|"[^"]*"|'[^']*'|[^\s,;&]+)/giu,
      "$1[redacted]",
    )
...
export function redactCredentialValues(value, redactionValues = []) {
  ...
  if (Array.isArray(value))
    return value.map((item) => redactCredentialValues(item, redactionValues));
  if (value !== null && typeof value === "object") {
    return Object.fromEntries(
      Object.entries(value).map(([key, item]) => [
        key,
        redactCredentialValues(item, redactionValues),
      ]),
    );
  }
  return value;
...
const redact = (value, redactionValues = []) =>
  redactText(value, redactionValues);
...
export function normalizeApiFailure(input, options = {}) {
  ...
  if (!Number.isInteger(status) || status < 0 || status > 599)
    fail("failure.status", "must be an HTTP status");
  let kind = value.kind;
  if (kind === undefined) {
    kind =
      status === 401
  ...
  if (value.retryable !== undefined && typeof value.retryable !== "boolean")
    fail("failure.retryable", "must be boolean");
  const risk = new Set(
    Array.isArray(value.risk_flags)
      ? value.risk_flags.map((item, index) =>
          enumValue(item, RISK_FLAGS, `failure.risk_flags[${index}]`),
        )
      : [],
  );
...
function safeValue(
  ...
  if (Array.isArray(value))
    return value.map((item, index) =>
      safeValue(item, `${path}[${index}]`, seen, redactionValues),
    );
  const result = {};
  for (const [key, item] of Object.entries(value)) {
    if (key === "__proto__" || key === "constructor" || key === "prototype")
      fail(`${path}.${key}`, "prototype-control keys are not allowed");
    if (
      /headers|authorization|secret|credential|client[_-]?id|api[-_]?key|token/iu.test(
        key,
      )
    )
      fail(`${path}.${key}`, "credential-bearing fields are not allowed");
...
export function normalizeRawEvidenceEnvelope(input, options = {}) {
  ...
  if (value.endpoint !== contract.endpoint)
    fail("evidence.endpoint", `must be ${contract.endpoint}`);
  if (method !== contract.method)
    fail("evidence.method", `must be ${contract.method} for ${source}`);
  ...
  const redactionValues = options?.redactValues ?? [];
  const requestInput = safeValue(
    object(value.request, "evidence.request"),
    "evidence.request",
    new Set(),
    redactionValues,
  );
  ...
  const request = Object.fromEntries(
    contract.requestFields
      .filter((field) => requestInput[field] !== undefined)
      .map((field) => [field, requestInput[field]]),
  );
  const result = {
    schema_version: 1,
  ...
  if (result.http.ok) {
    if (http.status !== 200)
      fail("evidence.http.status", "successful evidence must use HTTP 200");
    if (value.error !== undefined)
      fail("evidence.error", "must be absent for successful evidence");
    if (value.response === undefined)
      fail("evidence.response", "is required for successful evidence");
    ...
    if (contract.isEmpty(response))
      fail("evidence.response", "empty response is not usable evidence");
    result.response = response;
  } else {
    if (http.status !== 0 && http.status < 400)
      fail(
        "evidence.http.status",
        "failed evidence must use HTTP 0 or a 4xx/5xx status",
      );
    if (value.response !== undefined)
      fail("evidence.response", "must be absent for failed evidence");
    if (value.error === undefined)
      fail("evidence.error", "is required for failed evidence");
    const error = normalizeApiFailure(
      {
        ...safeValue(value.error, "evidence.error", new Set(), redactionValues),
        status: http.status,
      },
      { redactValues: redactionValues },
    );
    if (http.status === 0 && error.kind !== "network_error")
      fail("evidence.error.kind", "HTTP 0 failures must be network_error");
    result.error = error;
  }
  return result;
}
...
export function normalizeWjKeywordRecord(input) {
  ...
  const result = {
    ...
    related_keywords: Array.isArray(value.related_keywords)
      ? value.related_keywords.map((item, index) =>
          normalizedText(item, `record.related_keywords[${index}]`),
        )
      : fail("record.related_keywords", "must be an array"),
    search_intent: enumValue(
      value.search_intent,
      SEARCH_INTENTS,
    ...
    source: Array.isArray(value.source)
      ? value.source.map((item, index) =>
          enumValue(item, [...SAFE_SOURCES], `record.source[${index}]`),
        )
      : fail("record.source", "must be an array"),
    ...
    risk_flags: Array.isArray(value.risk_flags)
      ? [
          ...new Set(
            value.risk_flags.map((item, index) =>
              enumValue(item, RISK_FLAGS, `record.risk_flags[${index}]`),
            ),
          ),
        ].sort()
      : fail("record.risk_flags", "must be an array"),
    evidence_available: value.evidence_available,
    status: enumValue(value.status, STATUS_VALUES, "record.status"),
  };
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/u.test(result.category))
    fail("record.category", "must be a lowercase kebab-case category");
  if (result.source.length === 0) fail("record.source", "must not be empty");
  if (typeof result.evidence_available !== "boolean")
    fail("record.evidence_available", "must be boolean");
  if (
    result.status === "ready-to-write" &&
    (!result.evidence_available ||
      result.related_keywords.length < 2 ||
      result.related_keywords.length > 5)
  )
    fail(
      "record",
      "ready-to-write requires evidence and 2 to 5 related keywords",
    );
  if (result.status === "written" && !result.evidence_available)
    fail("record", "written requires evidence");
  return result;
}
...
export function isKeywordProvider(value) {
  return (
    ...
    typeof value.searchBlogs === "function" &&
    typeof value.searchTrends === "function"
  );
}

export function normalizeKeywordProvider(provider) {
  if (!isKeywordProvider(provider))
    fail("provider", "must expose providerId, searchBlogs, and searchTrends");
  return Object.freeze({
    providerId: provider.providerId,
    async searchBlogs(request) {
      const result = await provider.searchBlogs(
        normalizeBlogSearchRequest(request),
      );
      return result &&
        typeof result === "object" &&
        typeof result.kind === "string"
        ? normalizeApiFailure(result)
        : normalizeBlogSearchResponse(result);
    },
    async searchTrends(request) {
      const result = await provider.searchTrends(
        normalizeTrendRequest(request),
      );
      return result &&
        typeof result === "object" &&
        typeof result.kind === "string"
        ? normalizeApiFailure(result)
        : normalizeTrendResponse(result);
    },
  });
}
...
export const isValidBlogSearchRequest = (value) => {
  try {
    normalizeBlogSearchRequest(value);
    return true;
  } catch {
    return false;
  }
};
export const isValidTrendRequest = (value) => {
  try {
    normalizeTrendRequest(value);
    return true;
  } catch {
    return false;
  }
};
export const isValidApiFailure = (value) => {
  try {
    normalizeApiFailure(value);
    return true;
  } catch {
    return false;
  }
};
export const isValidRawEvidenceEnvelope = (value) => {
  try {
    normalizeRawEvidenceEnvelope(value);
    return true;
  } catch {
    return false;
  }
};
export const isValidWjKeywordRecord = (value) => {
  try {
    normalizeWjKeywordRecord(value);
    return true;
  } catch {
    return false;
  }
