import { DEFAULT_STOPWORDS, inferSearchIntent, normalizeKeyword } from './discovery.mjs';
...
const AUTO_STOPWORDS = Object.freeze([
  '방법', '정리', '사용법', '추천', '후기', '소개', '관련', '정보', '가이드',
  '뉴스', '이슈', '이야기', '알아보기', '시작하기', '활용', '경험', '생각',
  '오늘', '이번', '최근', '좋은', '통해', '위한', '대해', '대한', '에서',
  '로', '으로', '를', '을', '이', '가', '은', '는', '에', '의', '와', '과',
  '부터', '까지', '하는', '하기', '합니다', '입니다',
]);
...
function splitTrailingParticle(token) {
  const suffixes = ['으로', '부터', '까지', '를', '을', '은', '는', '이', '가', '에', '의', '와', '과', '도', '만', '로'];
...
function phraseWindows(text, stopwords) {
  const tokens = normalizeKeyword(text).split(' ').filter(Boolean).map(splitTrailingParticle);
  const phrases = [];
  for (let start = 0; start + MIN_TOPIC_WORDS <= tokens.length; start += 1) {
    for (let size = MIN_TOPIC_WORDS; size <= MAX_TOPIC_WORDS && start + size <= tokens.length; size += 1) {
 ...
 */
export function extractTopicCandidates({ category, query, response }, { limit = 5 } = {}) {
  if (typeof category !== 'string' || category.trim() === '') fail('category is required');
  if (typeof query !== 'string' || query.trim() === '') fail('query is required');
  if (!Number.isInteger(limit) || limit < 1 || limit > 20) fail('limit must be an integer from 1 to 20');

  const queryTokens = new Set(normalizeKeyword(query).toLowerCase().split(' ').filter(Boolean));
  const stopwords = new Set([...DEFAULT_STOPWORDS, ...AUTO_STOPWORDS, ...queryTokens]);
  const stats = new Map();
  ...
  normalizeItems(response).forEach((item, itemIndex) => {
    ...
    uniquePhrases.forEach((phrase) => {
      ...
      stats.set(key, {
        ...
        first_position: Math.min(previous.first_position, normalizedText.indexOf(phrase)),
      });
    });
  });
  ...
  const candidates = [...stats.values()]
    ...
    .sort((left, right) => (
      ...
      || normalizeKeywordKey(left.phrase).localeCompare(normalizeKeywordKey(right.phrase), 'ko')
    ))
    .slice(0, limit);
 ...
 */
export function buildAutomaticSeedDocument(discovered, { relatedLimit = 5 } = {}) {
  if (!Array.isArray(discovered) || discovered.length === 0) fail('discovered topics are required');
  if (!Number.isInteger(relatedLimit) || relatedLimit < 1 || relatedLimit > 5) fail('relatedLimit must be an integer from 1 to 5');

  const inputs = discovered.flatMap((group) => {
    if (!group || typeof group !== 'object' || typeof group.category !== 'string' || !Array.isArray(group.topics) || group.topics.length === 0) {
      fail('each automatic discovery group must contain category and topics');
    }
    const topics = group.topics.map((topic) => topic.topic).filter((topic) => typeof topic === 'string' && topic.trim() !== '');
    return group.topics.map((topic, index) => {
      const related = topics.filter((value) => value !== topic.topic).slice(0, relatedLimit);
      return {
