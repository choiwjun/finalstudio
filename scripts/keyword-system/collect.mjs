...
async function selectFixtureFile(fixture, source) {
  ...
  if (!isDirectory) {
    ...
    const siblingName =
      source === "blog" ? "blog-success.json" : "trend-success.json";
  ...
  for (const name of candidates)
    if (names.includes(name)) return join(value, name);
  const sourceMatch = names.find(
    (name) => name.toLowerCase().includes(source) && name.endsWith(".json"),
  );
...
function repositoryRelativePath(filePath, outDir) {
  ...
  if (
    ...
    return cwdRelative.split(sep).join("/");
  const outputParts = resolve(outDir).split(sep).filter(Boolean);
  const marker = outputParts.findIndex(
    (part, index) => part === "data" && outputParts[index + 1] === "keywords",
  );
  if (marker >= 0) {
...
function assertUniqueCandidates(candidates) {
  ...
  for (const candidate of candidates) {
    ...
    if (seen.has(key))
      fail(
        `duplicate candidate in one collection run: ${candidate.category}/${candidate.head_keyword}`,
      );
    seen.add(key);
  }
}
...
function candidateIdentity(candidate) {
  return `${normalizeKeywordKey(candidate.category)}\u0000${normalizeKeywordKey(candidate.head_keyword)}`;
}
...
async function markCandidatesResearching(candidates, outDir, collectedAt) {
  ...
  const existingMap = new Map(
    existing.map((record) => [candidateIdentity(record), record]),
  );
  for (const candidate of candidates) {
    const previous = existingMap.get(candidateIdentity(candidate));
    if (
      previous?.status === "researching" ||
      ["written", "rejected", "ready-to-write"].includes(previous?.status)
    )
      continue;
    const researching = {
      ...candidate,
    ...
    await upsertRecords([researching], {
      path: recordsPath,
      decisionsPath,
...
async function collectOne({
  ...
  const requests = [
    ...
    },
    {
      ...
      method: "POST",
      request: dateRequest(new Date(collectedAt), candidate),
      call: () =>
        provider.searchTrends(dateRequest(new Date(collectedAt), candidate)),
    },
  ];
  const traces = [];
  let failures = 0;
  for (const item of requests) {
    ...
    const empty =
      !failed &&
      ((item.source === "naver-api-hub-blog" &&
    ...
    if (dryRun) {
      traces.push({
        source: item.source,
        outcome: failed ? "failure" : empty ? "empty" : "success",
      });
      continue;
    }
    ...
    const relativeRaw = evidenceRelativePath({
      collectedAt,
      runId,
      source: item.source,
    ...
    if (seenTargets.has(targetKey))
      fail(
        `duplicate evidence target in one run: ${item.source}/${scopedSafeKey}`,
      );
    seenTargets.add(targetKey);
    if (await pathExists(target))
      fail(
        `evidence target already exists for this run: ${item.source}/${scopedSafeKey}`,
      );
    let persisted;
    try {
      persisted =
        failed || empty
          ? await writeFailureEvidence({
              ...
              http:
                failed && Number.isInteger(result.status) && result.status > 0
              ...
              error: failed
                ...
                : {
                    kind: "api_error",
                    message: "empty response is not usable evidence",
      ...
      const indexEntry = {
        ...
        path: repositoryRelativePath(persisted.path, outDir),
      };
      const indexPath = await assertContainedPath(
        resolve(outDir, "evidence-index.jsonl"),
        outDir,
      );
      await appendEvidenceIndexEntry({ path: indexPath, entry: indexEntry });
      traces.push({
        source: item.source,
        outcome: empty ? "failure" : indexEntry.outcome,
        path: indexEntry.path,
        run_id: indexEntry.run_id,
        collected_at: indexEntry.collected_at,
      });
    } catch {
      ...
      if (persisted?.path)
        await removeVerifiedFile(persisted.path).catch(() => {});
      failures += 1;
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (args.dryRun) {
    ...
    console.log(
      `collected ${candidates.length} candidate(s), 0 successful evidence call(s) (dry-run)`,
    );
    return {
      ...args,
      candidates,
      traces: candidates.map((candidate) => ({
        candidate: {
          ...
          head_keyword: candidate.head_keyword,
        },
        evidence: [
          { source: "naver-api-hub-blog", outcome: "planned" },
          { source: "naver-api-hub-trend", outcome: "planned" },
        ],
      })),
    };
  }
  ...
  const redactionValues = [
    providerEnv.NCP_NAVER_API_HUB_CLIENT_ID,
    providerEnv.NCP_NAVER_API_HUB_CLIENT_SECRET,
  ...
  for (const candidate of candidates) {
    ...
    traces.push({
      ...
      evidence: result.traces,
    });
    failures += result.failures;
  }
  const candidatesPath = await assertContainedPath(
    resolve(args.outDir, "candidates.json"),
    args.outDir,
  );
  const collectionPath = await assertContainedPath(
    resolve(args.outDir, "collection.json"),
    args.outDir,
  );
  await assertContainedPath(
    resolve(args.outDir, "evidence-index.jsonl"),
    args.outDir,
  );
  await writeBoundedJson(candidatesPath, candidates);
  await writeBoundedJson(collectionPath, {
    schema_version: 1,
    run_id: runId,
    collected_at: collectedAt,
    candidates: traces,
  ...
  console.log(
    `collected ${candidates.length} candidate(s), ${traces.reduce((sum, item) => sum + item.evidence.filter((evidence) => evidence.outcome === "success").length, 0)} successful evidence call(s); outputs data/keywords/raw, data/keywords/evidence-index.jsonl, data/keywords/collection.json`,
  );
...
if (
  ...
  main().catch((error) => {
    console.error(
      `keyword collection failed (${error?.code ?? "COLLECT_CLI"})`,
    );
    process.exitCode = 1;
  });
}
