...
test("review approval binds to the exact brief hash", () => {
  ...
  assert.throws(
    () => requireReviewedBriefHash(hash.toUpperCase(), hash),
    DraftBridgeError,
  );
  assert.throws(
    () => requireReviewedBriefHash("b".repeat(64), hash),
    DraftBridgeError,
  );
  assert.throws(
    () => requireReviewedBriefHash("not-a-hash", hash),
    DraftBridgeError,
  );
});
...
test("writer subprocess environment preserves bounded generation settings", () => {
  assert.deepEqual(
    buildWriterEnvironment({
      AUTO_BEST_OF: "1",
      AUTO_MAX_PASSES: "1",
      AUTO_ENHANCE_PASSES: "0",
      DATABASE_URL: "must-not-be-inherited",
    }),
    { AUTO_BEST_OF: "1", AUTO_MAX_PASSES: "1", AUTO_ENHANCE_PASSES: "0" },
  );
});
...
test("standalone auto-write rejects before reaching Codex", () => {
  ...
  assert.match(
    ...
    /approved keyword draft bridge/iu,
  );
  assert.doesNotMatch(
    `${result.stdout}${result.stderr}`,
    /test-(?:client|secret)-sentinel/iu,
  );
});
...
test("writer output is restricted to the content post directory", () => {
  ...
  assert.equal(
    parseDraftPath(
      "[convert-post] 저장 완료: /repo/src/content/posts/selected.md",
      { repositoryRoot: root, postsRoot: posts },
    ),
    "/repo/src/content/posts/selected.md",
  );
  assert.equal(
    buildWriterReference(root, "/repo/src/content/posts/selected.md"),
    "src/content/posts/selected.md",
  );
  assert.throws(
    () =>
      parseDraftPath("[convert-post] 저장 완료: /repo/out/escape.md", {
        repositoryRoot: root,
        postsRoot: posts,
      }),
    DraftBridgeError,
