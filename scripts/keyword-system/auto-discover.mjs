...
import { main as collectMain, createFixtureFetch, writeBoundedJson } from './collect.mjs';
...
function parsePositiveInteger(value, flag, maximum) {
  ...
  if (!Number.isInteger(parsed) || parsed < 1 || parsed > maximum) fail(`${flag} must be an integer from 1 to ${maximum}`);
  return parsed;
}
...
export function parseArgs(argv = []) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (flag === '--max-candidates') result.maxCandidates = parsePositiveInteger(value, flag, 20);
  }
  return result;
}
...
async function discoverFromNaver(args) {
  ...
  for (const categoryQuery of AUTO_CATEGORY_QUERIES) {
    ...
    if (response && typeof response.kind === 'string') {
      fail(`NAVER topic discovery failed for ${categoryQuery.category}: ${response.message ?? response.kind}`);
    }
    const topics = extractTopicCandidates({ ...categoryQuery, response }, { limit: args.maxCandidates });
    if (topics.length === 0) fail(`NAVER returned no usable topic candidates for ${categoryQuery.category}`);
    groups.push({ category: categoryQuery.category, topics });
    manifestGroups.push({
      category: categoryQuery.category,
      query: categoryQuery.query,
      result_count: Number.isInteger(response.total) ? response.total : response.items.length,
      candidates: topics,
      source: {
...
export async function main(argv = process.argv.slice(2), { clock = () => new Date() } = {}) {
  const args = parseArgs(argv);
  await assertSafeOutputDir(args.outDir);
  if (args.dryRun) {
    const categories = AUTO_CATEGORY_QUERIES.map((item) => ({ ...item }));
    console.log(`planned automatic topic discovery for ${categories.length} NAVER category query(ies); no network call or file write`);
  ...
  const seedPath = await assertContainedPath(resolve(args.outDir, 'automatic-seeds.json'), args.outDir);
  const manifestPath = await assertContainedPath(resolve(args.outDir, 'automatic-discovery.json'), args.outDir);
  ...
  await writeBoundedJson(manifestPath, {
    ...
    ranking_note: 'discovery_score is an internal extraction rank, not search volume, popularity, traffic, or revenue',
  ...
  console.log(`automatically discovered ${candidateCount} topic candidate(s); canonical evidence collection completed`);
  return { ...args, seedPath, manifestPath, seedDocument, collection };
}

if (process.argv[1] && fileURLToPath(import.meta.url) === resolve(process.argv[1])) {
  main().catch((error) => {
    console.error(`automatic topic discovery failed (${error?.code ?? 'AUTO_DISCOVER_CLI'})`);
    process.exitCode = 1;
  });
}
