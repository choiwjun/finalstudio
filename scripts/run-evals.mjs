...
const label =
  getArg("label") ??
  (existsSync(join(ROOT, "out", "evals", "baseline.json"))
    ? "after"
    : "before");

const fail = (msg) => {
  console.error(`[run-evals] 오류: ${msg}`);
  process.exit(1);
};
...
const parseJsonLine = (line, index) => {
  try {
    ...
    fail(
      `평가 케이스 ${index + 1}행이 올바른 JSON이 아닙니다 (${error instanceof Error ? error.message : String(error)})`,
    );
  }
};
...
const NOTES_REQUIRED = new Set(
  manifest.generationGate?.notesRequiredFormats ?? [],
);
...
const runCodex = (system, user) =>
  new Promise((resolveP, rejectP) => {
    ...
    child.on("close", (code) =>
      code === 0
        ? resolveP(out)
        : rejectP(new Error(`codex 실패 (exit ${code})\n${err.slice(-400)}`)),
    );
...
const stripReport = (t) =>
  t
    .replace(
      /\n*(?:-{3,}\s*윤문 리포트\s*-{3,}|#{1,4}\s*윤문 리포트)[\s\S]*$/,
      "",
    )
    .trim();
const parseScore = (t) =>
  Number(t.match(/총점:\s*(\d{1,3})\s*\/?\s*100/)?.[1] ?? NaN);
...
if (cases.length === 0)
  fail(`평가 케이스를 찾을 수 없습니다 (id=${onlyId ?? "전체"})`);
...
console.log(
  `[run-evals] 라벨 ${label} / 케이스 ${selected.length}건 — out/evals/${runId}/`,
);
...
for (const item of selected) {
  ...
  if (!blueprintPath) {
    results.push({ id: item.id, error: `블루프린트 없음: ${format}` });
    continue;
  }
  let notes;
  if (NOTES_REQUIRED.has(format)) {
    ...
    if (!existsSync(resolve(ROOT, fixture))) {
      results.push({ id: item.id, error: `원자료 fixture 없음: ${fixture}` });
      continue;
    }
    notes = read(fixture);
  }
  ...
  const writerInput = [
    ...
    `문체: 합니다체`,
    notes
      ? "\n--- 원자료 (유일한 사실·경험 재료 — 여기에 없는 경험·수치·장면을 만들지 마세요) ---"
      : undefined,
    notes,
  ]
  ...
  try {
    ...
    writeFileSync(
      join(outDir, `${item.id}-02-humanized.md`),
      candidate,
      "utf8",
    );
    ...
    while (passes < MAX_PASSES) {
      ...
      writeFileSync(
        join(outDir, `${item.id}-judge-${passes}.md`),
        judge,
        "utf8",
      );
      console.log(
        `  판정 ${passes}/${MAX_PASSES}: 기계 실패 ${analysis.failures.length}건 / 심사 ${Number.isNaN(score) ? "?" : score}점`,
      );
      ...
      if (passes < MAX_PASSES) {
        ...
        const fixedAnalysis = analyzePost(fixedText, {
          format,
          expectedMarkers,
          notes,
        });
        const better =
          fixedAnalysis.failures.length < analysis.failures.length ||
          (fixedAnalysis.failures.length === analysis.failures.length &&
            fixedAnalysis.pass &&
            !analysis.pass);
        if (!better) {
    ...
    const enhancePasses = Math.max(
      0,
      Number(manifest.generationGate?.enhancePasses ?? 0),
    );
    for (let e = 1; mechanicalPass && e <= enhancePasses; e++) {
      const enhanced = await runCodex(
        writerSystem,
        [
          candidate,
      ...
      const enhancedText = enhancedRaw
        ? stripReport(enhancedRaw)
        : stripReport(stripFences(enhanced));
      ...
      if (!enhancedAnalysis.pass) {
        ...
        writeFileSync(
          join(outDir, `${item.id}-enhance-${e}-rejected.md`),
          enhancedText,
          "utf8",
        );
        break;
      }
      const enhancedJudge = stripFences(
        await runCodex(
          judgeSystem,
          `SELECTED_FORMAT: ${format}\n\n${enhancedText}`,
        ),
      ...
      writeFileSync(
        join(outDir, `${item.id}-enhance-${e}.md`),
        enhancedText,
        "utf8",
      );
      writeFileSync(
        join(outDir, `${item.id}-enhance-${e}-judge.md`),
        enhancedJudge,
        "utf8",
      );
      console.log(
        `  강화 ${e}/${enhancePasses}: 심사 ${Number.isNaN(enhancedScore) ? "?" : enhancedScore}점 (이전 ${Number.isNaN(score) ? "?" : score}점)`,
      );
...
writeFileSync(
  join(ROOT, "out", "evals", `eval-${label}.json`),
  JSON.stringify(summary, null, 2),
  "utf8",
);
if (label === "before")
  writeFileSync(
    join(ROOT, "out", "evals", "baseline.json"),
    JSON.stringify({ runId, label }, null, 2),
    "utf8",
  );
...
if (label === "after" && beforePath && existsSync(beforePath)) {
  ...
  const comparison = results.map((r) => {
    ...
    const scoreDelta =
      (Number.isFinite(r.judgeScore) ? r.judgeScore : 0) -
      (Number.isFinite(p.judgeScore) ? p.judgeScore : 0);
    const failuresDelta =
      (r.mechanicalFailures ?? 0) - (p.mechanicalFailures ?? 0);
  ...
  writeFileSync(
    ...
    JSON.stringify({ comparison, regression }, null, 2),
    "utf8",
  );
  console.log("\n[run-evals] 전후 비교:");
  for (const c of comparison)
    console.log(
      `  ${c.id}: 점수 ${c.scoreDelta >= 0 ? "+" : ""}${c.scoreDelta}, 기계 실패 ${c.failuresDelta >= 0 ? "+" : ""}${c.failuresDelta}${c.regression ? " ← 회귀" : ""}`,
    );
}
...
if (regression) {
  console.error(
    "[run-evals] 회귀가 감지됐습니다. 프롬프트 변경을 재검토하세요.",
  );
