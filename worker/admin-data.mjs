...
const TOPIC_VALUES = new Set(["economy-business", "ai", "travel"]);
...
function validateBody(body) {
  if (typeof body !== "string" || body.length > MAX_BODY_CHARS)
    throw new Error("bodyMarkdown is invalid");
  if (body.replace(/\s/g, "").length === 0)
    throw new Error("bodyMarkdown is empty");
}
...
export function validateAdminPost(input) {
  ...
  const topic = textField(input.topic, "topic", 100);
  if (!TOPIC_VALUES.has(topic))
    throw new Error("topic must be economy-business, ai, or travel");
  ...
  const testedAt = optionalDate(
    input.testedAt ?? metadata.testedAt,
    "testedAt",
  );
  const publishAt = optionalDate(input.publishAt, "publishAt");
  if (status === "scheduled" && !publishAt)
    throw new Error("scheduled posts require publishAt");
  if (status !== "draft" && (!testedAt || author === "TBD"))
    throw new Error("public posts require testedAt and a real author");
  if (status !== "draft" && bodyMarkdown.replace(/\s/g, "").length < 1500)
    throw new Error(
      "public posts require at least 1,500 non-whitespace characters",
    );
  if (
    status !== "draft" &&
    /\[(?:직접 확인 필요|출처 URL 확인 필요|테스트 필요|스크린샷)[^\]]*\]/.test(
      bodyMarkdown,
    )
  )
    throw new Error("public posts must resolve review markers");
  ...
  if (!DATE_RE.test(normalizedMetadata.pubDate))
    throw new Error("pubDate is invalid");
  return {
    slug,
