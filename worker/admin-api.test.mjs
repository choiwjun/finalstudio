...
async function login(worker) {
  const response = await worker.fetch(new Request("https://wjblog.example/api/admin/login", {
    method: "POST",
    headers: { "Content-Type": "application/json", Origin: "https://wjblog.example" },
...
test("requires authentication before returning admin data", async () => {
  ...
  const response = await worker.fetch(new Request("https://wjblog.example/api/admin/posts"), env);
...
test("logs in, lists admin data, and saves a validated draft", async () => {
  ...
  const list = await worker.fetch(new Request("https://wjblog.example/api/admin/posts", { headers: { Cookie: cookie } }), env);
  const save = await worker.fetch(new Request("https://wjblog.example/api/admin/posts", {
    method: "PUT",
    headers: { Cookie: cookie, "Content-Type": "application/json", Origin: "https://wjblog.example" },
    body: JSON.stringify({
      slug: "admin-post",
      title: "Admin post",
      description: "Description",
...
test("rejects cross-origin admin writes and wrong passwords", async () => {
  const worker = createWorker({ connect: () => sqlStub });
  const wrong = await worker.fetch(new Request("https://wjblog.example/api/admin/login", {
    method: "POST",
    headers: { "Content-Type": "application/json", Origin: "https://wjblog.example" },
    body: JSON.stringify({ password: "wrong-password-123456" }),
  }), env);
  const crossOrigin = await worker.fetch(new Request("https://wjblog.example/api/admin/login", {
    method: "POST",
